...
public class Appels_abscences extends Activity implements OnGestureListener, TextToSpeech.OnInitListener, OnClickListener, OnLongClickListener{
	private TextToSpeech tts;
	private TextView appel_abscence;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		titre.setText("Appels en abscences");

		appel_abscence = (TextView) findViewById(R.id.tv_app);
		appel_abscence.setText("");
			...
			public boolean onDoubleTap(MotionEvent e) 
			{
				if(nom!=null){
					speakOut(nom+" , "+SimpleDateFormat.getDateInstance(
						...
						.format(callDayTime).toUpperCase().toString());
				}
				else{
	...
	public void onInit(int status) {
		if (status == TextToSpeech.SUCCESS) {
			...
			speakOut("Appels en abscences, défiler pour parcourir");
				...
				if(managedCursor.moveToNext()){
					...
					while(managedCursor.moveToNext() && !found){
						...
						if(dircode == CallLog.Calls.MISSED_TYPE){
							found = true;
							String isCallNew = managedCursor.getString(managedCursor.getColumnIndex(CallLog.Calls.NEW));
							if(Integer.parseInt(isCallNew)>0){
								ContentValues values = new ContentValues();
								values.put(CallLog.Calls.NEW,  0 );
								getContentResolver().update(CallLog.Calls.CONTENT_URI, values, isCallNew, null);
							}
	...
	public void onLongPress(MotionEvent arg0) {
		...
		Contacts.deactivate();
		Intent int_choix = new Intent(Appels_abscences.this, Menu_choix_journal.class);
